...
function App() {
  ...
  const [currentView, setCurrentView] = useState('books'); // 'books', 'loans', or 'bookDetail'
  ...
  function handleShowBookDetails(book) {
  ...
  function getBooks(bookData) {
    ...
    const isOnLoan = loans.some(loan => 
      (loan.book.isbn13 || loan.book.title) === (bookData.isbn13 || bookData.title)
    );
    return (
      <Book 
  ...
  function handleUpdateBook(updatedBook) {
    ...
    const isOnLoan = loans.some(loan => 
      (loan.book.isbn13 || loan.book.title) === (updatedBook.isbn13 || updatedBook.title)
    );
    
    if (isOnLoan) {
      alert("This book is currently on loan and cannot be edited. Please return it first.");
      return;
    }
  ...
  function handleDeleteBook() {
    ...
    // Check if book is currently on loan
    const isOnLoan = loans.some(loan => 
      (loan.book.isbn13 || loan.book.title) === (selectedBook.isbn13 || selectedBook.title)
    );
    
    if (isOnLoan) {
      alert("This book is currently on loan and cannot be deleted. Please return it first.");
      return;
    }
  ...
  return (
    <div>
      ...
      <main className="app-main">
        ...
        ) : currentView === 'bookDetail' && bookDetailData ? (
